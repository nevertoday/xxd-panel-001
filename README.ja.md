<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 001 プロジェクトバナー" width="1200">
</p>

<div align="center">

# 🦁 XXD Panel 001｜レトロ無邪気手帳

### 写真を、軽やかで素朴な紙の息づかいを持つレトロ手描き編集画へ
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 001 作例 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 001 作例 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 001 作例 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 001 作例 8"></td>
  </tr>
</table>

## 追加 3:4 上下構成作例

上の 16:9 作例と同じ4点の独立素材を、完全な 3:4 上下構成として再生成しました。上部は現実の写真を保ち、下部は Panel 001 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 001 追加上下作例 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 001 追加上下作例 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 001 追加上下作例 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 001 追加上下作例 4"></td>
  </tr>
</table>
...
> 素朴な線 · レトロな紙肌 · 混合画材 · 気の利いた比喩 · 温かな余白

XXD Panel 001 は、Codex と互換 Agent のための画像生成 Skill です。写真の人物や物、身振り、関係を識別できるまま残し、少し揺れる細線、水彩とガッシュ、色鉛筆、パステルやクレヨンが温かな繊維紙に重なる、軽やかなレトロ編集イラストへ翻訳します。
...
**Panel 001** は、原写真の核を保ちながら、次の視覚方向を目指す制作に向いています：**写真を、軽やかで素朴な紙の息づかいを持つレトロ手描き編集画へ**。まず用途と期待する結果を確認し、そこから構図、サイズ、文字、納品形式を決めます。
...
> XXD Panel 001 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
## すぐに判断：XXD Panel 001 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 写真を、軽やかで素朴な紙の息づかいを持つレトロ手描き編集画へ |
| **ひと目で分かる特徴** | 素朴な線 · レトロな紙肌 · 混合画材 · 気の利いた比喩 · 温かな余白 |
...
無関係な写真に替えても主体、比喩、身振り、配色、文字がほぼ成立するなら、それは 001 ではありません。

## 完成作品で見分けやすいスタイル特性

- **識別できる簡略化：** 元写真固有の手掛かりを最低3つ残します。比率は少し誇張しても、人、物、関係を汎用記号に置き換えません。
- **写真固有の変換：** 動き、構造、環境、感情から、自然で気の利いた一つの視覚的比喩を導き、固定の仕掛けを当てません。
- **人の手を感じる細線：** 線は細く、力が抜け、少し揺れ、途切れます。滑らかなベクター輪郭にはしません。
- **軽い混合画材：** 透明な水彩のにじみ、マットなガッシュ、色鉛筆のハッチング、パステルやクレヨンの粒、ドライブラシを薄く共存させます。
- **元写真由来の限定色：** 高明度・低〜中彩度の少ない色数を写真から導き、小さな補色アクセントを一つだけ置きます。
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 001 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-001 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-001 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-001 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-001 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 001 を適用し、共通設定は一度だけ解決
/xxd-panel-001 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを `~/.claude/skills/xxd-panel-001` にリンクできます。インストール後に Agent セッションを再起動してください。
...
- [中国語ランタイムアダプター](references/xxd-panel-001-prompt.zh-CN.md)
- [英語ランタイムアダプター](references/xxd-panel-001-prompt.en.md)
...
**写真の人と出来事を残し、力の抜けた一本の手描きで、もっと正確に語る。**
