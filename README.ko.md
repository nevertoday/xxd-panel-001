<p align="center">
  <img src="./assets/banner.svg" alt="XXD Panel 001 프로젝트 배너" width="1200">
</p>

<div align="center">

# 🦁 XXD Panel 001｜레트로 천진 저널

### 사진을 가볍고 소박하며 종이의 숨결이 느껴지는 레트로 손그림 편집 일러스트로
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 001 예시 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 001 예시 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 001 예시 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 001 예시 8"></td>
  </tr>
</table>

## 추가 3:4 상하 구성 예시

위 16:9 예시와 같은 네 개의 독립 소스를 완전한 3:4 상하 캔버스로 다시 생성했습니다. 위쪽은 현실 사진을 유지하고 아래쪽은 Panel 001의 원본 프롬프트에 따라 재구성합니다. 영어 문구는 각 사진의 내용·분위기·함의에서만 생성했으며 모든 결과를 개별 검수했습니다.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 001 추가 상하 예시 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 001 추가 상하 예시 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 001 추가 상하 예시 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 001 추가 상하 예시 4"></td>
  </tr>
</table>
...
> 소박한 선 · 레트로 종이결 · 혼합 매체 · 재치 있는 은유 · 따뜻한 여백

XXD Panel 001은 Codex와 호환 에이전트를 위한 이미지 생성 Skill입니다. 사진 속 인물과 사물, 동작과 관계를 알아볼 수 있게 남긴 뒤, 조금 흔들리는 가는 선과 수채·과슈·색연필·파스텔 또는 크레용이 따뜻한 섬유질 종이에 놓인 가벼운 레트로 편집 일러스트로 번역합니다.
...
**Panel 001**은 원본 사진의 핵심을 보존하면서 다음과 같은 시각 방향을 원하는 작업에 적합합니다: **사진을 가볍고 소박하며 종이의 숨결이 느껴지는 레트로 손그림 편집 일러스트로**. 먼저 용도와 원하는 결과를 확인한 뒤 구성, 크기, 텍스트와 전달 형식을 결정합니다.
...
> XXD Panel 001으로 이 이미지를 작업하고, 먼저 가장 어울리는 구성과 크기를 추천해 줘.
...
## 빠른 판단: XXD Panel 001이 나에게 맞을까요?

| 궁금한 점 | 이 스타일이 주는 답 |
|---|---|
| **얻게 되는 결과** | 사진을 가볍고 소박하며 종이의 숨결이 느껴지는 레트로 손그림 편집 일러스트로 |
| **한눈에 알아볼 특징** | 소박한 선 · 레트로 종이결 · 혼합 매체 · 재치 있는 은유 · 따뜻한 여백 |
...
무관한 사진으로 바꿔도 주제, 은유, 동작, 팔레트, 문구가 거의 그대로 성립한다면 001이 아닙니다.

## 완성작에서 바로 알아볼 수 있는 스타일 특징

- **알아볼 수 있는 단순화:** 원본 고유 단서를 최소 세 가지 남깁니다. 비율은 조금 과장할 수 있지만 인물, 사물, 관계를 흔한 기호로 바꾸지 않습니다.
- **사진 고유의 전환:** 동작, 구조, 환경, 감정에서 자연스럽고 영리한 시각적 은유 하나를 도출하며 고정된 장치를 씌우지 않습니다.
- **사람 손의 흔들림이 있는 가는 선:** 선은 가늘고 느슨하며 조금 흔들리고 끊깁니다. 매끈한 벡터 윤곽을 피합니다.
- **가벼운 혼합 매체:** 투명한 수채 번짐, 무광 과슈, 색연필 해칭, 파스텔이나 크레용 입자, 드라이 브러시를 얇게 공존시킵니다.
- **원본 기반 제한 팔레트:** 사진에서 가져온 고명도·저~중채도 색을 제한적으로 쓰고 작은 보색 포인트 하나만 둡니다.
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 001의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-001 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-001 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-001 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-001 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-001 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-001 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 001 적용, 공통 설정은 한 번만 확정
/xxd-panel-001 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 `~/.claude/skills/xxd-panel-001`에 연결할 수 있습니다. 설치 후 에이전트 세션을 다시 시작하세요.
...
- [Skill 워크플로](SKILL.md)
- [중문 런타임 어댑터](references/xxd-panel-001-prompt.zh-CN.md)
- [영문 런타임 어댑터](references/xxd-panel-001-prompt.en.md)
...
**사진 속 사람과 순간을 남기고, 힘을 뺀 한 번의 손그림으로 더 정확하게 말하세요.**
