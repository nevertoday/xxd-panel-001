---
name: xxd-panel-001
description: "Create XXD Panel 001 raster artwork while treating the bundled original style brief as the sole aesthetic authority. Supports one or more top-bottom, left-right, design-only, and four-device wallpaper outputs; multiple ratios or exact sizes; prompt-generated, user-exact, or text-free typography; and inline parameters. Use when the user invokes xxd-panel-001 or requests the exact Panel 001 style."
---

# XXD Panel 001

Create fresh PNG artwork with `references/001-source.md` as the **sole creative and aesthetic authority**. This Skill is a runtime shell: it resolves delivery variables, appends them to the untouched source-brief body, executes bitmap generation, and verifies the result. It must not write a second art direction.

## Authority boundary

1. Read `references/001-source.md` completely immediately before building every generation request.
...
```text
/xxd-panel-001 <source> --mode top-bottom,design-only \
...
```text
[VERBATIM SOURCE-BRIEF BODY FROM references/001-source.md,
...
- the result follows `references/001-source.md`, especially its own colour, material, composition, whitespace, and typography requirements, without an outer Skill palette or added art direction;
...
These files document the same minimal adapter. They never replace `references/001-source.md`.
